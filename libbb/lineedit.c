...
#if ENABLE_UNICODE_SUPPORT
static void FAST_FUNC read_key_ungets_unicode(char *buffer, const char *str, unsigned len)
{
...
static void FAST_FUNC bs_to_slash_unicode(wchar_t *p)
{
	for (; *p; ++p) {
		if ( *p == L'\\') {
			*p = L'/';
...
static int lineedit_read_key(char *read_key_buffer, int timeout)
{
 ...
 pushback:
				...
				read_key_ungets_unicode(read_key_buffer, unicode_buf + 1, unicode_idx - 1);
# if !ENABLE_UNICODE_PRESERVE_BROKEN
...
#if ENABLE_PLATFORM_MINGW32
		case CTRL('Z'):
			command_ps[command_len] = BB_NUL;
#if ENABLE_UNICODE_SUPPORT
			bs_to_slash_unicode(command_ps);
#else
