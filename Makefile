...
PATCHLEVEL = 31
SUBLEVEL = 0
EXTRAVERSION = .$(shell git describe --tags)
NAME = Unnamed
...
RCS_FIND_IGNORE := \( -name SCCS -o -name BitKeeper -o -name .svn -o -name CVS -o -name .pc -o -name .hg -o -name .git -o -name liblongpath-win32 \) -prune -o
...
quiet_cmd_busybox__ ?= LINK    $@
      ...
      "$(LDFLAGS) $(EXTRA_LDFLAGS) $(WRAP_FILEIO)" \
      "$(core-y)" \
      "$(libs-y)" \
      "$(LDLIBS)" \
